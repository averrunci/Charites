...
public abstract class ControllerCollection<TElement> : Collection<object> where TElement : class
{
    ...
    public void AttachTo(TElement element)
    {
        ...
        if (associatedElement is not null) throw new InvalidOperationException("Associated element must be null.");
    ...
    public void Detach()
    {
        if (associatedElement is null) return;
    ...
    protected void AttachExtensions(object controller)
    {
        if (associatedElement is null) return;

        extensions.ForEach(extension => AttachExtension(extension, controller, associatedElement));
    }
    ...
    /// </summary>
    /// <param name="controller">The controller in which extensions are defined.</param>
    protected void DetachExtensions(object controller)
    {
        if (associatedElement is null) return;

        extensions.ForEach(extension => DetachExtension(extension, controller, associatedElement));
    }
