...
namespace Charites.Windows.Mvc
{
    ...
    class ControllerCollectionSpec : FixtureSteppable
    {
        ...
        object[] ActualControllers { get; } = { new object(), new object(), new object() };
        TestControllers.DisposableController DisposableController { get; } = new TestControllers.DisposableController();
        ...
        void Ex01()
        {
            ...
            Then("the DataContext should not be found", () => DataContextFinder.DidNotReceive().Find(Element1));
            Then("the DataContext should not be injected", () => DataContextInjector.DidNotReceive().Inject(DataContext, ActualControllers[0]));
            Then("the Element should not be injected", () => ElementInjector.DidNotReceive().Inject(Element1, ActualControllers[0]));
            Then("the Extensions should not be attached", () => Extensions.ForEach(extension => extension.DidNotReceive().Attach(ActualControllers[0], Element1)));
        ...
        void Ex04()
        {
           ...
           ClearReceivedCalls();
            ...
            Then("the DataContext should not be initialized", () => DataContextInjector.DidNotReceive().Inject(null, ActualControllers[0]));
            Then("the Element should not be initialized", () => ElementInjector.DidNotReceive().Inject(null, ActualControllers[0]));
            Then("the Extensions should not be detached", () => Extensions.ForEach(extension => extension.DidNotReceive().Detach(ActualControllers[0], Element1)));
        ...
        void Ex10()
        {
            ...
            Then("the DataContext should not be initialized", () => DataContextInjector.DidNotReceiveWithAnyArgs().Inject(null, null));
            Then("the Element should not be initialized", () => ElementInjector.DidNotReceiveWithAnyArgs().Inject(null, null));
            Then("the Extensions should not be detached", () => Extensions.ForEach(extension => extension.DidNotReceiveWithAnyArgs().Detach(null, Element1)));
        }
    }
}
