...
class ControllerTypeFinderSpec : FixtureSteppable
{
    ...
    TestElement Element { get; } = new(string.Empty);
    IEnumerable<Type> ControllerTypes { get; set; } = default!;
    ...
    void Ex03()
    {
        ...
        When("the element that has the key and has the base type is specified", () => ControllerTypes = ControllerTypeFinder.Find(new DerivedTestElement(string.Empty)));
